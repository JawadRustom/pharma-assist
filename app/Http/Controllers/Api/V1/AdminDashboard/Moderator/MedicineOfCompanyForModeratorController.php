...
use Illuminate\Http\Request;
    ...
    /**
     * @group Admin Medicine Of Copmany
     *
     * This Api For Medicine Of Copmany
     */

class MedicineOfCompanyForModeratorController extends Controller
{
    /**
     * See all Medicine inside one Copmany
     ...
     *
     */
    public function seeMedicineOfCompanyForModerator(Request $request, $name)
    {
        $user = auth()->user();
        $data = Medicine::orderBy('id', 'desc')
            ->whereHas('companies', fn ($query) => $query->where('name', $name))
